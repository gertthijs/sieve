...
Calculator::ParseFilterFile(const std::string& filename, Options* o)
{
   ...
   while (pfs.good())
   {
      ...
      if (parameter == "SDFTAG")
      {
         if (!std::strncmp(o->InputConverter()->GetInFormat()->GetID(), "sdf", 3))
         {
...
//      if (!o->Salts()) (void) mol.StripSalts();  // Use again this function as of OB 2.3.1
      ...
      for (unsigned int i = 0; i < _elementFilters.size(); ++i)
      {
         ...
         if (_ef->IsInitialised())
         {
            ...
            else if (_ef->HasFailed())
            {
               ...
               if (!o->FailFilename().empty()) o->FailConverter()->Write(&mol, o->FailStream());
               goto endOfLoop2;
            }
         }
      }
      ...
      else 
      {
         ...
         o->PassConverter()->Write(&mol, o->PassStream());
         mol.Clear();
         continue;
